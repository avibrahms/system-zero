...
SKIP_REASONS = {
    ...
    "solo-venture": "operator venture portfolio; excluded from public reconstruction",
    "viralepic": "operator product module; excluded from public reconstruction",
}

MODULE_BEHAVIORS: dict[str, dict[str, Any]] = {
    "action-card-cleanup-ce": {
        "summary": "Archives completed action-card items from shared storage and removes them from the active card.",
        "behaviors": [
        ...
        "provides": "ce.action.card.cleanup",
        "setpoints": {},
    },
    "agent-dashboard-ce": {
        "summary": "Builds a dashboard health snapshot from generic session, action-card, and module state files.",
        "behaviors": [
        ...
        "provides": "ce.agent.dashboard",
        "setpoints": {},
    },
    "check-linkedin-autopost-policy-ce": {
        "summary": "Checks generic content auto-post policy files for stale eligibility and cadence language.",
        "behaviors": [
        ...
        "provides": "ce.content.autopost.policy",
        "setpoints": {},
    },
    "chronicle-ce": {
        "summary": "Records recent bus events into a hashed daily chronicle stream.",
        "behaviors": [
        ...
        "provides": "ce.chronicle",
        "setpoints": {},
    },
    "context-assembler-ce": {
        "summary": "Selects the highest-signal repository files for a task or focus query.",
        "behaviors": [
        ...
        "provides": "ce.context.assembler",
        "setpoints": {
            "max_tokens": {
                "default": 2000,
                "range": [100, 20000],
                "description": "Approximate context budget for selected files.",
                "mode": "simple",
            },
            "top_n": {
                "default": 10,
    ...
    "eidetic-ce": {
        "summary": "Indexes evidence-like repository documents into a compact memory map.",
        "behaviors": [
        ...
        "provides": "ce.eidetic.index",
        "setpoints": {},
    },
    "mcp-server-launcher-ce": {
        "summary": "Discovers MCP server definitions and produces a launch readiness plan.",
        "behaviors": [
        ...
        "provides": "ce.mcp.launch.plan",
        "setpoints": {},
    },
    "queue-gate-ce": {
        "summary": "Counts active ready-queue entries and opens or closes a content-production gate.",
        "behaviors": [
        ...
        "provides": "ce.queue.gate",
        "setpoints": {
            "max_depth": {
                "default": 15,
                "range": [0, 500],
                "description": "Maximum active queue entries before the gate closes.",
                "mode": "simple",
            }
        },
    },
    "registry-validator-ce": {
        "summary": "Validates the installed System Zero registry, manifests, capabilities, and config.",
        "behaviors": [
        ...
        "provides": "ce.registry.validation",
        "setpoints": {},
    },
    "rollback-email-verification-ce": {
        "summary": "Removes generic email verification artifacts from shared System Zero storage.",
        "behaviors": [
        ...
        "provides": "ce.email.verification.rollback",
        "setpoints": {},
    },
    "sentinel-ce": {
        "summary": "Scans repository files for common secret, config, and safety findings.",
        "behaviors": [
        ...
        "provides": "ce.sentinel.scan",
        "setpoints": {},
    },
    "session-bootstrap-ce": {
        "summary": "Builds a deterministic session-start payload from repo profile, git state, and module health.",
        "behaviors": [
        ...
        "provides": "ce.session.bootstrap",
        "setpoints": {},
    },
    "skill-library-ce": {
        "summary": "Exposes the sanitized recursive-question skill catalog as a queryable module.",
        "behaviors": [
        ...
        "provides": "ce.skill.library",
        "setpoints": {},
    },
    "spec-dependency-graph-ce": {
        "summary": "Builds a dependency graph from phase and workflow specification documents.",
        "behaviors": [
        ...
        "provides": "ce.spec.dependency.graph",
        "setpoints": {},
    },
    "spec-lint-ce": {
        "summary": "Lints System Zero plan and protocol specs for required sections and stale placeholders.",
        "behaviors": [
        ...
        "provides": "ce.spec.lint",
        "setpoints": {},
    },
    "start-preamble-ce": {
        "summary": "Generates a compact start-preamble from current session and reference-stack state.",
        "behaviors": [
        ...
        "provides": "ce.start.preamble",
        "setpoints": {},
    },
    "system-zero-ce": {
        "summary": "Audits the local System Zero installation and reconstructed stack readiness.",
        "behaviors": [
...
def text_files(path: Path) -> list[Path]:
    if path.is_file():
        return [path]
    ignored_suffixes = {".png", ".jpg", ".jpeg", ".pdf", ".docx", ".DS_Store", ".pyc", ".gz"}
    files = []
...
def extract_description(path: Path, module_id: str) -> str:
    ...
    return MODULE_BEHAVIORS.get(module_id, {}).get(
        "summary", f"{title_for(module_id)} organ reconstructed from connection-engine source."
    )


def capability_name(module_id: str) -> str:
    spec = MODULE_BEHAVIORS.get(module_id) or {}
    return str(spec.get("provides") or ("ce." + module_id.replace("-ce", "").replace("-", ".")))
...
def manifest_for(module_id: str) -> dict[str, Any]:
    spec = MODULE_BEHAVIORS[module_id]
    return {
        "id": module_id,
        "version": "0.1.0",
        "category": "connection-engine-skills" if module_id == "skill-library-ce" else "connection-engine",
        ...
        "requires": [{"providers": ["bus", "memory", "discovery", "storage"]}],
        "setpoints": spec.get("setpoints", {}),
    ...
    contract.update(
        {
            ...
            "interfaces": ["bus", "memory", "discovery", "storage"],
            "public_purpose": spec["summary"],
            "behaviors": spec["behaviors"],
            "reconstruction": "Protocol-native behavior was re-derived from the source candidate and hardened into an anonymized System Zero module.",
            "absorb_workflow": {
                "status": "materialized_with_functional_hardening",
                "source": "sz absorb draft plus deterministic protocol adapter",
            },
        }
    )
...
On every tick this module emits `{event_type(module_id)}` with module-specific outcome fields and records the same outcome in the `ce.reconstruction` memory stream.
...
def main() -> int:
    ...
    for item in inventory:
        ...
        if item["category"] == "rewrite":
            skill_path = source_path if source_path.name == "SKILL.md" else source_path / "SKILL.md"
            if skill_path.exists():
                ...
                item["phase16_note"] = "rewritten into skill-library-ce as a sanitized public skill contract"
            else:
                item["category"] = "skip"
        ...
        if module_id not in MODULE_BEHAVIORS:
            item["category"] = "skip"
            item["reconstruction_status"] = "skipped"
            item["phase16_note"] = "source candidate needs a bespoke anonymized adapter before public reconstruction"
            skipped.append({"id": source_id, "reason": item["phase16_note"]})
            continue
        contract = {
            "source_kind": item["kind"],
            "source_label": rel_source(source_path),
            "source_summary": extract_description(source_path, module_id),
        ...
        item["phase16_note"] = "absorbed via protocol draft and hardened into a functional anonymized module"
    ...
    if skill_items:
        skill_contract = {
            ...
            "source_summary": "Expose the reusable recursive-question skill set as an anonymized System Zero module.",
            "source_metrics": {
        ...
        json.dumps(
            {
                ...
                "failed": [],
                "proof": {
                    "method": "sz absorb protocol draft plus deterministic module-specific hardening",
                    "module_specific_behavior": True,
                    "generic_snapshot_wrappers": False,
                },
            },
            indent=2,
        )
